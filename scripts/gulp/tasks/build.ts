...
import { DIST_BUILD_ROOT, DIST_BUILD_ES2015_ROOT, DIST_BUILD_UMD_ROOT, ES5, ES_2015, PROJECT_ROOT, UMD_MODULE, DIST_BUILD_FESM_ROOT } from '../constants';
...
export function buildIonicAngularFesm(stripDebug: boolean, done: Function) {
  const stream = copySourceToDest(DIST_BUILD_FESM_ROOT, true, true, stripDebug);
  stream.on('end', () => {
    // the source files are copied, copy over a tsconfig from
    createTempTsConfig([], ES5, ES_2015, `${PROJECT_ROOT}/tsconfig.fesm.json`, `${DIST_BUILD_FESM_ROOT}/tsconfig.json`);
    runNgc(`${DIST_BUILD_FESM_ROOT}/tsconfig.json`, (err) => {
      if (err) {
        done(err);
        return;
      }
